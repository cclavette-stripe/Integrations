...
function connectReader(discoveredReaders) {
...
function discoverReaderHandler() {
  var config = { simulated: false, location: "tml_EkQ8cQj8fYewDb" };
  terminal.discoverReaders(config).then(function (discoverResult) {
    if (discoverResult.error) {
      ...
      console.log("Discovered reader(s): ", discoveredReaders);
    }
  });
}
...
connectButton.addEventListener("click", async (event) => {
  connectReader(discoveredReaders);
});
